  ...
  methods: {
    async getAttendanceData() {
      try {
        ...
        this.labels = response.data.map((item) => `${item.name} (${this.formattedDate(item.date)})`);
...
<template>
  ...
  <div class="grid grid-cols-2 gap-4">
    ...
    <div class="bg-gray-100 p-4 text-center">
      <h2 class="text-2xl">Event Attendance</h2>
    ...
    <div class="bg-gray-100 p-4 text-center">
      <h2 class="text-2xl">Clients by Zip Code</h2>
      <donutChart v-if="!loading && !error" :label="labels" :chart-data="chartData"></donutChart>
