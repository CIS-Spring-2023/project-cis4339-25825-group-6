...
export default {
  ...
  created() {
    axios.get(`${apiURL}/events/id/${this.$route.params.id}`).then((res) => {
      console.log(res);
      this.event = res.data;
      this.event.services = res.data.services;
    ...
    axios.get(`${apiURL}/services/services`).then((res) => {
      this.selectedServices = res.data;
    });
  },
  methods: {
    ...
    toggleSelection(serviceName) {
      if (this.event.services.includes(serviceName)) {
        this.event.services = this.event.services.filter((name) => name !== serviceName);
      } else {
        this.event.services.push(serviceName);
      }
      ...
      <form @submit.prevent="handleSubmitForm">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
          ...
          <div class="flex flex-col">
            <label class="block">
              ...
              <span style="color: #ff0000">*</span>
              <input type="date" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" v-model="event.date" />
          ...
          <div class="flex flex-col grid-cols-3">
            ...
            <div v-for="item in selectedServices" :key="item">
              <label class="inline-flex items-center">
                <input
                  type="checkbox"
                  :value="item.name"
                  @click="toggleSelection(item.name)"
                  :checked="event.services.includes(item.name)"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
          <div class="flex justify-between mt-10 mr-20">
            <button @click="handleEventUpdate" type="submit" class="bg-green-700 text-white rounded">Update Event</button>
          </div>
          <div class="flex justify-between mt-10 mr-20">
            <button @click="eventDelete" type="submit" class="bg-red-700 text-white rounded">Delete Event</button>
          </div>
          <div class="flex justify-between mt-10 mr-20">
